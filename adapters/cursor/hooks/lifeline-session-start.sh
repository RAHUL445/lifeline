#!/usr/bin/env bash
# lifeline @persist_state binding extra (Cursor FULL tier): sessionStart hook.
# Two jobs:
...
# Fallback harnesses rely on the resuming-a-cycle / using-lifeline skills instead.

set -euo pipefail

# Drain stdin (Cursor sends the event JSON; we only need the repo on disk)
cat >/dev/null || true

emit() { # $1 = additional_context string
