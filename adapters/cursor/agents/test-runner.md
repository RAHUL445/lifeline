...
isolated subagent. Resolve the plugin root from the `${CURSOR_PLUGIN_ROOT}` environment
variable.

1. Read `${CURSOR_PLUGIN_ROOT}/core/contracts/TEST_RESULT.md` (payload + classification rules)
   and `${CURSOR_PLUGIN_ROOT}/core/skills/coverage-to-smoke/SKILL.md` step 1 (coverage pass);
   follow both exactly.
